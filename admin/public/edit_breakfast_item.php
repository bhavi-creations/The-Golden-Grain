...
// Fetch existing breakfast item
$stmt = $conn->prepare("SELECT * FROM breakfast_items WHERE item_id = ?");
$stmt->bind_param("i", $item_id);
$stmt->execute();
$result = $stmt->get_result();
if ($result->num_rows === 0) {
    $_SESSION['msg_error'] = "Breakfast item not found.";
    header("Location: all_breakfast_items.php");
    exit();
}
...
// --- Step 1: Map old status values to Active/Inactive ---
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = $_POST['status']; // will be Active / Inactive
    $price = $_POST['price'];
    ...
    // Update database
    $stmt = $conn->prepare("UPDATE breakfast_items SET menu_category_id=?, item_name=?, veg_nonveg=?, status=?, price=?, photo=? WHERE item_id=?");
    $stmt->bind_param("isssdsi", $menu_category_id, $item_name, $veg_nonveg, $status, $price, $photo_filename, $item_id);

    if ($stmt->execute()) {
        $_SESSION['msg_success'] = "Breakfast item updated successfully!";
    } else {
...
<head>
    <meta charset="utf-8">
    <title>Edit Breakfast Item - The Golden Grain</title>
        ...
        <div id="content" class="flex-fill d-flex flex-column">
            <div class="container-fluid py-4 flex-fill d-flex flex-column">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4 flex-wrap">
                    <h1 class="h3 mb-0 text-gray-800">Edit Breakfast Item</h1>
                    <a href="all_breakfast_items.php" class="btn btn-secondary btn-sm shadow-sm">
                            ...
                            <div class="card-body">
                                <form method="POST" enctype="multipart/form-data">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="item_name" id="item_name" class="form-control" required value="<?= htmlspecialchars($item['item_name']) ?>">
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="veg_nonveg" id="veg_nonveg" class="form-control" required>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" step="0.01" name="price" id="price" class="form-control" required value="<?= htmlspecialchars($item['price']) ?>">
                                    </div>

                                    <div class="mb-3">
                                        <label for="photo" class="form-label">Photo</label>
                                        <input type="file" name="photo" id="photo" class="form-control">
                                        <?php if (!empty($item['photo']) && file_exists(__DIR__ . "/uploads/breakfast_items/" . $item['photo'])) : ?>
                                            <img src="uploads/breakfast_items/<?= $item['photo'] ?>" alt="Current Photo" class="img-fluid mt-2" style="max-height: 150px;">
                                        <?php endif; ?>
                                    </div>
