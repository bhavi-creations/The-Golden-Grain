...
// Fetch existing item
$stmt = $conn->prepare("SELECT * FROM lunch_items WHERE item_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update database
    $stmt = $conn->prepare("UPDATE lunch_items SET menu_category_id = ?, item_name = ?, veg_nonveg = ?, status = ?, price = ?, photo = ? WHERE item_id = ?");
    $stmt->bind_param("isssdsi", $menu_category_id, $item_name, $veg_nonveg, $status, $price, $photo_filename, $item_id);

    if ($stmt->execute()) {
        $_SESSION['msg_success'] = "Lunch item updated successfully!";
    } else {
...
<head>
    <meta charset="utf-8">
    <title>Edit Lunch Item - The Golden Grain</title>
            ...
            <div id="content" class="flex-fill d-flex flex-column">
                <div class="container-fluid py-4 flex-fill d-flex flex-column">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4 flex-wrap">
                        <h1 class="h3 mb-0 text-gray-800">Edit Lunch Item</h1>
                        <a href="all_lunch_items.php" class="btn btn-secondary btn-sm shadow-sm">
                                ...
                                <div class="card-body">
                                    <form method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <?php if (!empty($item['photo']) && file_exists(__DIR__ . "/uploads/lunch_items/" . $item['photo'])) : ?>
                                                <img src="uploads/lunch_items/<?= $item['photo'] ?>" alt="Current Photo" class="img-fluid mt-2" style="max-height: 150px;">
                                            <?php endif; ?>
                                        </div>
